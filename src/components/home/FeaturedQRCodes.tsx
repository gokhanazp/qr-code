...
// Ana sayfada müşterilerin QR kodlarını gösteren basit bileşen
// (Simple component to display customer QR codes on homepage)
  ...
  return (
    <section className="py-20 bg-gradient-to-b from-gray-50 to-white overflow-hidden">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        {/* QR Kodları Grid - Sadece QR kod gösterimi */}
        {loading ? (
          ...
          <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
            {qrCodes.map((qr) => {
              const Icon = typeIcons[qr.type.toLowerCase()] || LinkIcon
              const gradient = typeColors[qr.type.toLowerCase()] || 'from-blue-500 to-indigo-500'
              const settings = qr.settings as Record<string, string>

              return (
                <div
                  key={qr.id}
                  className="group bg-white rounded-2xl p-4 shadow-lg hover:shadow-2xl transition-all duration-300 hover:-translate-y-1 border border-gray-100"
                >
                  {/* QR Kod */}
                  <div className="relative">
                    <div className="w-full aspect-square bg-gradient-to-br from-gray-50 to-white rounded-xl p-3 border border-gray-100 shadow-inner">
                      <QRCodeSVG
                        value={getQRValue(qr)}
                        size={200}
                        level="M"
                    ...
                    {/* Tip badge */}
                    <div className={`absolute -top-2 -right-2 w-8 h-8 bg-gradient-to-br ${gradient} rounded-xl flex items-center justify-center shadow-lg`}>
